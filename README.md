...
### Лицензирование PyQt6

PyQt6 доступен по [GPLv3 либо коммерческой лицензии Riverbank](https://www.riverbankcomputing.com/software/pyqt/intro/). У этого
репозитория пока нет выбранной проектной лицензии, поэтому текущая версия
предназначена для внутренней исследовательской работы и запуска из исходников.
До публичного распространения исходников или бинарной сборки владелец проекта
должен принять и оформить один из двух вариантов: сделать весь распространяемый
продукт GPLv3-совместимым либо приобрести подходящую коммерческую лицензию
PyQt6. Отсутствие файла `LICENSE` не даёт третьим лицам разрешения на
распространение этого кода.
...
### Что появилось в исследовательском UI

- Верхний баннер явно различает `DEMO`, устаревший кадр, ошибку, `ARMED` и
  запись опыта. В `DEMO` подключение и отправка на физический робот запрещены
  не только кнопкой, но и worker-контуром.
- Для MP4 доступен timeline: play/pause, шаг на ±1 кадр, пять скоростей и
  выключенный по умолчанию цикл A/B. Любой временной скачок прекращает отправку
  на робот и требует нового ручного arm. Для камеры вместо timeline показан
  честный статус `LIVE`.
- Вкладка «Робот» показывает авторитетный checklist готовности, а не повторяет
  проверки по цветам виджетов. При `ARMED` постоянно видны endpoint, частота,
  возраст команды, число успешных отправок и кнопка «Остановить отправку — не
  E-stop».
- Вкладка «Эксперимент» пишет воспроизводимый пакет `manifest.yaml +
  events.jsonl + chunks/*.npz`. Запись разрешается только production-pipeline
  после свежего кадра и завершённой калибровки. Видео выключено по умолчанию и
  требует отдельного согласия; сбой MP4-кодека не останавливает числовую запись.
- Журнал поддерживает порог уровня, поиск, сортировку, автопрокрутку, JSON-
  детали и экспорт всех либо только отфильтрованных событий.

Пакеты опытов находятся в стандартной папке документов ОС:

```text
HumanoidInterface/experiments/<YYYY-MM-DD>/<run_id>/
```

До успешной финализации каталог имеет суффикс `.partial`. После следующего
запуска незавершённые `.partial` помечаются как `interrupted` и не удаляются;
уже финализированные неполные/ошибочные пакеты сохраняют исходную причину и
время завершения. Раскладка splitters, выбранная вкладка и обычные параметры
камеры сохраняются через Qt; `ARMED`, interlock и активная запись никогда не
восстанавливаются.
